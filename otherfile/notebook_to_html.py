...
NOTEBOOKS_DICT = {"Static":["Amphlett","Chamberline_Kim","Larminie_Dicks"], "Dynamic":["Chakraborty","Padulles_Amphlett","Padulles_Hauer","Padulles1","Padulles2"]}
...
if __name__ == "__main__":
    ...
    tprint("Document Convert", "bulbhead")
    print("Processing ...")
    ...
    for folder in sorted(NOTEBOOKS_DICT):
        ...
        for index, notebook in enumerate(NOTEBOOKS_DICT[folder]):
            notebook_path = os.path.join("Documents", notebook + NOTEBOOK_EXTENSION)
            notebook_copy_path = os.path.join(folder_path, notebook + NOTEBOOK_EXTENSION)
            html_file_path = os.path.join(folder_path, notebook + HTML_EXTENSION)
            shutil.copy(notebook_path, notebook_copy_path)
            ep = ExecutePreprocessor(timeout=6000, kernel_name='python3')
            with open(notebook_copy_path, "r", encoding="utf-8") as f:
                nb = nbformat.read(f, as_version=4)
                ep.preprocess(nb, {'metadata': {'path': folder_path, 'title': notebook}})
            with open(notebook_copy_path, 'w', encoding='utf-8') as f:
                nbformat.write(nb, f)
            exporter = HTMLExporter()
            output_notebook = nbformat.read(notebook_copy_path, as_version=4)
            output, resources = exporter.from_notebook_node(output_notebook, {'metadata': {'name': notebook}})
